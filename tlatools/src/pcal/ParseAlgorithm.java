   ...
   public static AST getAlgorithm(PcalCharReader charR, boolean fairAlgorithm) throws ParseAlgorithmException
     ...
     **********************************************************************/
     { try {
       ...
       // returns only the algorithm name
       //System.out.println("GetAlgToken:::: " + "\n" + name + "\n"); print what GetAlgToken is returning - prints algo name
       ...
       TLAExpr defs = new TLAExpr();
		if (PeekAtAlgToken(1).equals("define")) {
			...
			GobbleThis(";");
			PcalDebug.reportInfo("define statements found and successfully added.");
		}
		...
		while (PeekAtAlgToken(1).equals("macro")) {
			...
			PcalDebug.reportInfo("macro statements found and successfully added.");
		}
		...
		while (PeekAtAlgToken(1).equals("procedure")) {
			...
			omitPC = false;
			PcalDebug.reportInfo("procedure statements found and successfully added.");
		}
		...
		if(PcalParams.distpcalFlag) {
			...
			if (PeekAtAlgToken(1).equals("node") || PeekAtAlgToken(1).equals("process")
				...
				PcalDebug.reportInfo("Distributed Pluscal Multi-nodes if statement");
				...
				PcalDebug.reportInfo("Distibured Pluscal multi-nodes algorithm process/node is found.");
				if (pSyntax) {
					GobbleThis("end");
				...
				// p-syntax error line is above.
				PcalDebug.reportInfo("Distibured Pluscal multi-nodes algorithm syntax is OK.");
				...
				while (i < multiNodes.nodes.size()) {
					...
					// next debug print omitStutteringWhenDoneValue
					...
					while (j < node.threads.size()) {
						...
						PcalDebug.reportInfo("Dist +Pcal multi-process inside thread while");
				...
				while (i < multiNodes.prcds.size()) {
					...
					PcalDebug.reportInfo("Dist +Pcal while (i < multiNodes.prcds.size() LOOP. ");
				...
				PcalDebug.reportInfo("Distributed PLuscal: Multi process declarations OK");
				return multiNodes;
			} // multi-process part end DISTRIBUTED PLUSCAL
			
			else { // uni process part begin DISTRIBUTED PLUSCAL
			...
			PcalDebug.reportInfo("Distributed Pluscal Uni process GetStmtSeq() is OK");
			CheckForDuplicateMacros(uniproc.macros) ;
			ExpandMacrosInStmtSeq(uniproc.body, uniproc.macros) ;
			PcalDebug.reportInfo("Distributed Pluscal Uni process ExpandMacrosInStmtSeq() is OK");
			
			// Dostonbek edit for expanding channel expression in uni-process specs
			...
			{
				PcalDebug.reportInfo("Some procedures found. Uni process Distributed Pluscal.");
				AST.Procedure prcd =
			...
			AddLabelsToStmtSeq(uniproc.body) ;
			PcalDebug.reportInfo("Distributed Pluscal Uni process AddLabelsToStmtSeq() is OK");
			uniproc.body = MakeLabeledStmtSeq(uniproc.body);
			PcalDebug.reportInfo("Distributed Pluscal Uni process MakeLabeledStmtSeq() is OK");
			/****************************************************************
			...
			uniproc.setOrigin(new Region(uniprocBegin, 
					GetLastLocationEnd())) ;
			PcalDebug.reportInfo("Distributed PLuscal: uni process declarations OK");
			return uniproc ;
			...
			if ((PeekAtAlgToken(1).equals("fair") 
				...
				PcalDebug.reportInfo("in multi process PLUSCAL");
			...
			// FOR UNIPROCESS ALGORITHM PLUSCAL			
			else
			{ 

			PcalDebug.reportInfo("in uni process PLUSCAL");
			AST.Uniprocess uniproc = new AST.Uniprocess() ;
			TLAtoPCalMapping map = PcalParams.tlaPcalMapping ;
			PCalLocation uniprocBegin = new PCalLocation(map.algLine, map.algColumn);
			uniproc.name   = name ;
  ...
  public static boolean inGetMacro = false ;
   ...
   public static Vector GetStmtSeq() throws ParseAlgorithmException 
     ...
     **********************************************************************/
     { Vector result = new Vector() ;
       String tok = PeekAtAlgToken(1) ;
       PcalDebug.reportInfo("GetStmtSeq tok: " + tok);
       while (! (    (    pSyntax
                      && (   tok.equals("end")
                          || tok.equals("else")
                          || tok.equals("elsif")
         ...
         { String nextLabel = GetLabel() ;
           ...
           if (cSyntax && PeekAtAlgToken(1).equals("{"))
             ...
             { /************************************************************
               ...
               ************************************************************/
               AST stmt = GetStmt() ;
               PcalDebug.reportInfo("Called GetStmt() method inside GetStmtSeq");
               stmt.lbl = nextLabel ;
	...
	public static AST GetStmt() throws ParseAlgorithmException {
		...
		PcalDebug.reportInfo("GetStmt. nextTok: " + nextTok);
...
***************************************************************************/
   ...
   public static Vector MakeLabeledStmtSeq(Vector stmtseq) throws ParseAlgorithmException 
     ...
     **********************************************************************/
     { 
	   PcalDebug.reportInfo("Called MakeLabeledStmtSeq ");
...
***************************************************************************/
   public static void CheckLabeledStmtSeq(Vector stmtseq) throws ParseAlgorithmException
     { 
	   PcalDebug.reportInfo("Inside CheckLabeledStmtSeq");
	   int i = 0 ;
       while (i < stmtseq.size())
         { AST.LabeledStmt stmt = (AST.LabeledStmt) stmtseq.elementAt(i) ;
         	PcalDebug.reportInfo("Make a call to ClassifyStmtSeq = " + i + " : times");
     ...
     { int i = 0 ;
       ...
       while (i < stmtseq.size())
         { 
    	   	 ...
    	   	 PcalDebug.reportInfo("ClassifyStmtSeq stmtseq: " + stmtseq);
    	   	 
             if (node.getClass().equals(
                           AST.LabelIfObj.getClass()))
               { 
            	 PcalDebug.reportInfo("ClassifyStmtSeq LabelIfObj if clause");
             ...
             // Dostonbek
             else if ( node.getClass().equals(AST.ChannelSenderObj.getClass()) ) {
            	 PcalDebug.reportInfo("Handle channel. ");
            	 result = 0;
            	 return result;
             }
             //Dostonbek end
               ...
               {
            	 PcalDebug.reportInfo("LabelEitherObj: ");
               ...
               {
            	 PcalDebug.reportInfo("ReturnObj || CallReturnObj ");
               ...
               {
            	 PcalDebug.reportInfo("GotoObj || CallGotoObj ");
            	 result = 1 ;
               }
        ...
        PcalDebug.reportInfo("result: " + result);
      return result ;
     }     
...
***************************************************************************/
   ...
   public static void ExpandMacrosInStmtSeq(Vector stmtseq, Vector macros) throws ParseAlgorithmException
     ...
     **********************************************************************/
     {
	   PcalDebug.reportInfo("ExpandMacrosInStmtSeq method. stmtseq: " + stmtseq.toString() + " macros: " + macros);
	   int i = 0 ;
       while (i < stmtseq.size())
         { 
    	   	 AST node = (AST) stmtseq.elementAt(i) ;
    	   	 //PcalDebug.reportInfo(node.toString());
             if (node.getClass().equals(
                           AST.LabelIfObj.getClass()))
               {
            	 PcalDebug.reportInfo("ExpandMacrosInStmtSeq inside if branchL LabelIfObj");
...
//      }
   ...
   public static AST.Node GetNode() throws ParseAlgorithmException {
		...
		if(PeekAtAlgToken(1).equals("process")) {
			...
			PcalDebug.reportInfo("AST.NODE node keyword found. ");
			GobbleThis("node");
		}
		...
		PcalDebug.reportInfo("AST.NODE process name 1st line built successfully. ");
		...
		if (PeekAtAlgToken(1).equals("begin") || PeekAtAlgToken(1).equals("{") || PeekAtAlgToken(1).equals("subprocess")) {
			result.decls = new Vector(1);
			PcalDebug.reportInfo("searching for begin or { or subprocess STATEMENT.");
		} else {
			// enters only if we declared local variables/channels inside process
			if(result.decls == null) {
				result.decls = new Vector();
				PcalDebug.reportInfo("Varible declarations inside process");
			}
			result.decls.addAll(GetVarDecls());
			result.decls.addAll(GetChannelDecls());
			PcalDebug.reportInfo("Varible declarations inside process2");
		}
		...
		*/
		...
		threads.add(thread);
		// c-syntax enters this the following if block of code. 
		if (PeekAtAlgToken(1).equals("{") || PeekAtAlgToken(1).equals("begin")) {
			...
			while (PeekAtAlgToken(1).equals("{") || PeekAtAlgToken(1).equals("subprocess")) {
				PcalDebug.reportInfo("Inside subprocess while loop.");
		...
		/* start of the subprocess - Heba's code
   ...
   public static Vector GetChannelDecls() throws ParseAlgorithmException {
	...
	public static AST.ChannelSendCall getSendToChannelCall(String nextTok) throws ParseAlgorithmException {
		PcalDebug.reportInfo("getSendToChannelCall : " + nextTok);
		...
		PcalDebug.reportInfo("result of getSendToChannelCall:  " + result);
		return result;
	}
	...
	public static void ExpandChannelCallersInStmtSeq(Vector stmtseq, Vector nodeDecls, Vector globalDecls) throws ParseAlgorithmException {
		PcalDebug.reportInfo("ExpandChannelCallersInStmtSeq. stmtseq " + stmtseq + " nodeDecls: " + nodeDecls +
		...
		while (i < stmtseq.size()) {
			...
			if (stmt.getClass().equals(AST.LabelIfObj.getClass())) {
				PcalDebug.reportInfo("ExpandChannelCallersInStmtSeq LabelIfObj if block");
				...
				PcalDebug.reportInfo("ExpandChannelCallersInStmtSeq LabelEitherObj if block");
				...
				PcalDebug.reportInfo("ExpandChannelCallersInStmtSeq WithObj if block");
				ExpandChannelCallersInStmtSeq(((AST.With) stmt).Do, nodeDecls, globalDecls);
			} else if (stmt.getClass().equals(AST.WhileObj.getClass())) {
				PcalDebug.reportInfo("ExpandChannelCallersInStmtSeq WhileObj if block");
				...
				PcalDebug.reportInfo("ExpandChannelCallersInStmtSeq ChannelSenderObj if block");
				...
				PcalDebug.reportInfo("ExpandChannelCallersInStmtSeq ChannelReceiverObj if block");
				...
				PcalDebug.reportInfo("ExpandChannelCallersInStmtSeq ChannelClearCall if block");
	...
	public static Vector ExpandSendCall(AST.ChannelSendCall call, Vector nodeDecls, Vector globalDecls) throws ParseAlgorithmException {
		PcalDebug.reportInfo("ExpandSendCall nodeDecls:" + nodeDecls);
        ...
        PcalDebug.reportInfo("ExpandSendCall return value: " + result);
		return result;
	}
