...
//        tlacode.addElement(is.toString());
		...
		if(decl instanceof AST.FIFOChannel || decl instanceof AST.UnorderedChannel
			...
			PcalDebug.reportInfo("Found channel/fifo declartion. Calling RewriteVarDeclDimensions method.");
...
/* -------------------------------------------------------------------------- */
	...
	private void RewriteVarDeclDimensions(Channel decl) {
		...
		// when channel/fifo do not have dimension: channel/fifo c;
		if ( decl.dimensions == null || decl.dimensions.size() == 0) {
			System.out.println("Building one-dimensional communication channel: simple {} or <<>>");
		...
		// when fifo/channel have dimensions: channel/fifo c[N]; c[N, N]
		else {
			System.out.println("Building Muti-dimensional communication channel");
